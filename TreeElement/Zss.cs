...
namespace TreeElement
{
    ...
    public abstract class Zss<T>
    {
        ...
        /// </summary>
        private Tuple<int, List<Tuple<ZssNode<T>, ZssNode<T>>>>[,] _treedists;
        ...
        private int[] ComputeL(int[] t1, List<ZssNode<T>>  tree)
        {
            ...
            foreach (var node in t1)
            {
                ...
                var leftMostDescendant = currentNode.GetLeftMostDescendant();
                result[node] = tree.IndexOf(leftMostDescendant) + 1;
            }
            return result;
        }
       ...
       /// sequence of edit operations</returns>
        public Tuple<int, List<Tuple<ZssNode<T>, ZssNode<T>>>>  Compute()
        {
            ...
            _treedists = new Tuple<int, List<Tuple<ZssNode<T>, ZssNode<T>>>>[T1.Length + 1, T2.Length + 1];
            _treedists[0, 0] = Tuple.Create(0,new List<Tuple<ZssNode<T>, ZssNode<T>>>());
        ...
        private void Treedists(int i, int j)
        {
            ...
            var fd = new Tuple<int,List<Tuple<ZssNode<T>, ZssNode<T>>>>[m, n];
            fd[0, 0] = Tuple.Create(0, new List<Tuple<ZssNode<T>, ZssNode<T>>>());
            var ioff = _l1[i] - 1;
            var joff = _l2[j] - 1;
            ...
            for (int x = 1; x < m; x++)
            {
                ...
                var edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x - 1, 0].Item2);
                //edits.Add("delete ZssNode: " + x);
                edits.Add(Tuple.Create<ZssNode<T>, ZssNode<T>>(A[x - 1], null));
                fd[x, 0] = Tuple.Create(fd[x - 1, 0].Item1 + 1, edits); 
            }
            for (int y = 1; y < n; y++)
            {
                var node = B[y - 1 + joff];
                var edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[0, y - 1].Item2);
                //edits.Add("insert ZssNode: " + node);
                edits.Add(Tuple.Create<ZssNode<T>, ZssNode<T>>(null, node));
            ...
            for (int x = 1; x < m; x++)
            {
                for (int y = 1; y < n; y++)
                {
                    if (_l1[i] == _l1[x + ioff] && _l2[j] == _l2[y + joff])
                    {
                        ...
                        List<Tuple<ZssNode<T>, ZssNode<T>>> edits;
                        if (value == fd[x - 1, y].Item1 + 1)
                        {
                            var node = A[x - 1];
                            //edits = new List<string>(fd[x - 1, y].Item2) {"remove ZssNode: " + node};
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x - 1, y].Item2) { Tuple.Create<ZssNode<T>, ZssNode<T>>(node, null) };
                        } else if (value == fd[x, y - 1].Item1 + 1)
                        {
                            var node = B[y - 1 + joff];
                            //edits = new List<string>(fd[x, y - 1].Item2) { "insert ZssNode: " + node };
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x, y - 1].Item2) { Tuple.Create<ZssNode<T>, ZssNode<T>>(null, node) };
                        }
                        else
                        {
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x - 1, y - 1].Item2); 
                            if (CostUpdate(A[x + ioff - 1], B[y + joff - 1]) > 0)
                            {
                                //edits.Add("update ZssNode: " + A[x + ioff - 1] + " to: " + B[y + joff - 1]);
                                edits.Add(Tuple.Create<ZssNode<T>, ZssNode<T>>(A[x + ioff - 1], B[y + joff - 1]));
                            }
                        }
                    ...
                    else
                    {
                        ...
                        {
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[p, q].Item2);
                            edits.AddRange(_treedists[x + ioff, y + joff].Item2);
                        }
                        else if (value == fd[x - 1, y].Item1 + 1)
                        {
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x - 1, y].Item2);
                            //edits.Add("delete ZssNode: " + A[x - 1]);
                            edits.Add(Tuple.Create < ZssNode<T>, ZssNode < T >>(A[x - 1], null));
                        }
                        else
                        {
                            edits = new List<Tuple<ZssNode<T>, ZssNode<T>>>(fd[x, y - 1].Item2);
                            //edits.Add("insert ZssNode: " + B[y - 1]);
                            edits.Add(Tuple.Create<ZssNode<T>, ZssNode<T>>(null, B[y - 1]));
                        }
                        fd[x, y] = Tuple.Create(value, edits);
                    }
                }
