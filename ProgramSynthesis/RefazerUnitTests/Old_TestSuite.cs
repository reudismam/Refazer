...
namespace RefazerUnitTests
{
    ...
    public class TestSuite
    {
        ...
        /************************************************************************************************************
         * Generate Training Database Giving The minimum amount of examples necessary to generate a correct program
         * **/
        ...
        public static bool CompleteTestBase(string commit, string solutionPath = null, List<SyntaxKind> kinds = null)
        {
            ...
            List<Tuple<ProgramNode, bool>> analyzedPrograms = new List<Tuple<ProgramNode, bool>>();
            int eIndex = 1;
            for (eIndex =1; eIndex < randomList.Count; eIndex++)
            {      
                var examples = randomList.GetRange(0, eIndex);
                ...
                analyzedPrograms = new List<Tuple<ProgramNode, bool>>();
                for (var i = 1; i <= allPrograms.Count; i++)
                {
                    ...
                    if (firstProblematicLocation == -1)
                    {
                        ...
                        if (firstIncorrect == -1)
                        {
                            //LogProgram(commit, i, p.ToString(), examples.Count, true);
                            analyzedPrograms.Add(Tuple.Create(p, true));
                        }
                        else
                        {
                            //LogProgram(commit, i, p.ToString(), examples.Count, false);
                            analyzedPrograms.Add(Tuple.Create(p, false));
                        }
                    }
                    else
                    {
                ...
                foreach (Tuple<ProgramNode, bool> tu in analyzedPrograms) {
                    if (tu.Item2)
                    {
                        atleastOneCorrect = true;
                    }
                }
                for (int pIndex = 0; pIndex < analyzedPrograms.Count; pIndex++)
                {
                    var p = analyzedPrograms[pIndex].Item1;
                    var status = analyzedPrograms[pIndex].Item2;
                    LogProgram(commit, pIndex, p.ToString(), eIndex, status);
                }
            ...
            //    {
            //        var p = analyzedPrograms[pIndex].Item1;
            //        LogProgram(commit, pIndex, p.ToString(), eIndex - 1, true);
            //    }
            //}
            return true;
        }
        ...
        public static void LogProgram(string commit, int programIndex, string program, int exTransformations, bool status)
        {
            string commitFirstLetter = commit.ElementAt(0).ToString();
            string commitId = commit.Substring(commit.IndexOf(@"\") + 1);
            ...
            using (ExcelManager em = new ExcelManager())
            {
                ...
                em.SetValue("B" + empty, programIndex);
                em.SetValue("C" + empty, program);
                em.SetValue("D" + empty, exTransformations);
                em.SetValue("E" + empty, status);
                em.Save();
            }
        }
    }
