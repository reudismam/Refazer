...
namespace LeastCommonAncestor
{
    class LCA<T>
    {
        static void Init(ListNode list)
        {
            ...
            TreeNode<SyntaxNodeOrToken> rootNode = ConvertToTreeNode(list.List[0]);
        ...
        private static TreeNode<SyntaxNodeOrToken> ConvertToTreeNode(SyntaxNodeOrToken st)
        {
            ...
            List<SyntaxNodeOrToken> childrens = new List<SyntaxNodeOrToken>();
            foreach (SyntaxNodeOrToken sot in st.ChildNodesAndTokens())
            {
                childrens.Add(sot);
                TreeNode<SyntaxNodeOrToken> nodes = ConvertToTreeNode();
            }*/
            return null;
        }
    }
